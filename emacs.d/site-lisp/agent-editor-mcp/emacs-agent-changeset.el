;;; emacs-agent-changeset.el --- Change tracking and rollback -*- lexical-binding: t; -*-

;;; Commentary:

;; One mutating tool call creates one runtime-scoped change set.  Before-images
;; remain in memory; journals receive only metadata and frozen diffs.

;;; Code:

(require 'cl-lib)
(require 'seq)
(require 'subr-x)
(require 'emacs-agent-document)
(require 'emacs-agent-policy)
(require 'emacs-agent-project)
(require 'emacs-agent-runtime)

(declare-function emacs-agent-document-revision-for-target
                  "emacs-agent-document" (runtime target))

(define-error 'emacs-agent-changeset-error "Emacs Agent change set error")
(define-error 'emacs-agent-rollback-conflict "Change set cannot be rolled back"
  'emacs-agent-changeset-error)

(defcustom emacs-agent-changeset-cursor-ttl 300
  "Seconds for which change-set list and detail cursors remain valid."
  :type 'integer
  :group 'emacs-agent-editor)

(defvar emacs-agent-changeset-cursors (make-hash-table :test #'equal)
  "Opaque immutable pagination snapshots for change-set queries.")

(cl-defstruct (emacs-agent-changeset
               (:constructor emacs-agent-changeset--make))
  changeset-id runtime-instance-id request-id agent-identity created-at status
  operations touched-documents base-revisions final-revisions before-snapshots
  checkpoint-state diagnostics-before diagnostics-after unified-diff
  insertions deletions)

(defun emacs-agent-changeset--id ()
  "Return a new opaque change-set ID."
  (emacs-agent-runtime--random-id "changeset"))

(defun emacs-agent-changeset--target (runtime path)
  "Resolve absolute PATH as a direct target in RUNTIME."
  (emacs-agent-project-resolve-target runtime path :for-create t))

(defun emacs-agent-changeset--canonical-path (runtime path)
  "Return the canonical absolute identity for PATH in RUNTIME."
  (emacs-agent-resolved-target-canonical-path
   (emacs-agent-changeset--target runtime path)))

(defun emacs-agent-changeset--canonical-entries (runtime entries)
  "Copy ENTRIES with path keys canonicalized through RUNTIME."
  (mapcar
   (lambda (entry)
     (cons
      (emacs-agent-changeset--canonical-path runtime (car entry))
      (copy-tree (cdr entry))))
   entries))

(defun emacs-agent-changeset--canonical-operation (runtime operation)
  "Copy OPERATION with direct path fields canonicalized in RUNTIME."
  (let ((copy (copy-tree operation)))
    (when (and (listp copy) (keywordp (car copy)))
      (dolist (key '(:path :new_path))
        (when-let* ((path (plist-get copy key)))
          (when (stringp path)
            (setq
             copy
             (plist-put
              copy key
              (emacs-agent-changeset--canonical-path
               runtime path)))))))
    copy))

(defun emacs-agent-changeset--snapshot-exists-p (snapshot)
  "Return whether SNAPSHOT represents an existing document."
  (if (stringp snapshot)
      t
    (plist-get snapshot :exists)))

(defun emacs-agent-changeset--snapshot-content (snapshot)
  "Return content stored in SNAPSHOT."
  (if (stringp snapshot)
      snapshot
    (or (plist-get snapshot :content) "")))

(defun emacs-agent-changeset--buffer-content (buffer)
  "Return the complete authoritative content of BUFFER."
  (with-current-buffer buffer
    (save-restriction
      (widen)
      (buffer-substring-no-properties (point-min) (point-max)))))

(defun emacs-agent-changeset--current-content (runtime path)
  "Return authoritative current content for canonical PATH in RUNTIME."
  (let* ((target (emacs-agent-changeset--target runtime path))
         (canonical
          (emacs-agent-resolved-target-canonical-path target))
         (registry
          (emacs-agent-runtime-document-registry runtime))
         (document (gethash canonical registry))
         (status (emacs-agent-document-status runtime target)))
    (setq document (or document (gethash canonical registry)))
    (cond
     ((and document
           (buffer-live-p (emacs-agent-document-buffer document)))
      (emacs-agent-changeset--buffer-content
       (emacs-agent-document-buffer document)))
     ((plist-get status :exists_on_disk)
      (emacs-agent-changeset--buffer-content
       (emacs-agent-document-buffer
        (emacs-agent-document-open runtime target))))
     (t ""))))

(defun emacs-agent-changeset--diff-text (path before after)
  "Create a unified diff for canonical PATH from BEFORE to AFTER."
  (if (equal before after)
      ""
    (let ((old-file (make-temp-file "emacs-agent-old-"))
          (new-file (make-temp-file "emacs-agent-new-"))
          (output (generate-new-buffer " *emacs-agent-diff*")))
      (unwind-protect
          (progn
            (with-temp-file old-file
              (insert before))
            (with-temp-file new-file
              (insert after))
            (if (executable-find "diff")
                (let ((status
                       (process-file
                        (executable-find "diff") nil output nil
                        "-u" "--label" (concat "a/" path)
                        "--label" (concat "b/" path)
                        old-file new-file)))
                  (unless (memq status '(0 1))
                    (signal
                     'emacs-agent-changeset-error
                     (list "Unable to generate unified diff")))
                  (with-current-buffer output
                    (buffer-string)))
              (format
               "--- a/%s\n+++ b/%s\n@@ complete file @@\n-%s\n+%s\n"
               path path before after)))
        (when (file-exists-p old-file)
          (delete-file old-file))
        (when (file-exists-p new-file)
          (delete-file new-file))
        (when (buffer-live-p output)
          (kill-buffer output))))))

(cl-defun emacs-agent-changeset-record
    (runtime &key request-id agent-identity operations touched-documents
             base-revisions final-revisions before-snapshots checkpoint-state
             diagnostics-before diagnostics-after unified-diff insertions
             deletions)
  "Record one completed mutation in RUNTIME and return its change set.
REQUEST-ID and AGENT-IDENTITY attribute OPERATIONS.  TOUCHED-DOCUMENTS,
BASE-REVISIONS, FINAL-REVISIONS, and BEFORE-SNAPSHOTS describe affected state.
CHECKPOINT-STATE records persistence.  DIAGNOSTICS-BEFORE and
DIAGNOSTICS-AFTER record analysis state.  UNIFIED-DIFF may supply a frozen
diff, while INSERTIONS and DELETIONS provide summary counts."
  (let* ((canonical-before
          (emacs-agent-changeset--canonical-entries
           runtime before-snapshots))
         (canonical-base
          (emacs-agent-changeset--canonical-entries
           runtime base-revisions))
         (canonical-final
          (emacs-agent-changeset--canonical-entries
           runtime final-revisions))
         (canonical-touched
          (mapcar
           (lambda (path)
             (emacs-agent-changeset--canonical-path
              runtime path))
           (or
            touched-documents
            (mapcar #'car canonical-before))))
         (canonical-operations
          (mapcar
           (lambda (operation)
             (emacs-agent-changeset--canonical-operation
              runtime operation))
           operations))
         (id (emacs-agent-changeset--id))
         (status
          (if (memq checkpoint-state '(t checkpointed))
              'checkpointed
            'applied))
         (changeset
          (emacs-agent-changeset--make
           :changeset-id id
           :runtime-instance-id
           (emacs-agent-runtime-instance-id runtime)
           :request-id request-id
           :agent-identity agent-identity
           :created-at (float-time)
           :status status
           :operations canonical-operations
           :touched-documents canonical-touched
           :base-revisions canonical-base
           :final-revisions canonical-final
           :before-snapshots canonical-before
           :checkpoint-state checkpoint-state
           :diagnostics-before diagnostics-before
           :diagnostics-after diagnostics-after
           :unified-diff
           (or
            unified-diff
            (mapconcat
             (lambda (entry)
               (let ((path (car entry))
                     (snapshot (cdr entry)))
                 (emacs-agent-changeset--diff-text
                  path
                  (emacs-agent-changeset--snapshot-content
                   snapshot)
                  (emacs-agent-changeset--current-content
                   runtime path))))
             canonical-before ""))
           :insertions (or insertions 0)
           :deletions (or deletions 0))))
    (puthash
     id changeset
     (emacs-agent-runtime-changeset-registry runtime))
    (emacs-agent-runtime-record-activity
     runtime
     (list
      :request_id request-id
      :tool "mutation"
      :status (symbol-name status)
      :changeset_id id
      :paths
      (emacs-agent-changeset-touched-documents changeset)))
    changeset))

(defun emacs-agent-changeset-get (runtime changeset-id)
  "Return CHANGESET-ID from RUNTIME, or signal."
  (or
   (gethash
    changeset-id
    (emacs-agent-runtime-changeset-registry runtime))
   (signal
    'emacs-agent-changeset-error
    (list (format "Unknown change set: %s" changeset-id)))))

(defun emacs-agent-changeset-list (&optional runtime)
  "Return change records from RUNTIME, newest first."
  (let ((runtime (or runtime (emacs-agent-runtime-current)))
        items)
    (maphash
     (lambda (_id changeset)
       (push changeset items))
     (emacs-agent-runtime-changeset-registry runtime))
    (sort
     items
     (lambda (left right)
       (>
        (emacs-agent-changeset-created-at left)
        (emacs-agent-changeset-created-at right))))))

(defun emacs-agent-changeset--revision (runtime path)
  "Return the current revision for canonical PATH in RUNTIME."
  (let* ((target (emacs-agent-changeset--target runtime path))
         (canonical
          (emacs-agent-resolved-target-canonical-path target))
         (status (emacs-agent-document-status runtime target))
         (document
          (gethash
           canonical
           (emacs-agent-runtime-document-registry runtime))))
    (if
        (or
         document
         (plist-get status :exists_on_disk)
         (plist-get status :visited))
        (emacs-agent-document-revision-for-target
         runtime target)
      nil)))

(defun emacs-agent-changeset--rollback-status (runtime changeset)
  "Return rollback availability metadata for CHANGESET in RUNTIME."
  (cond
   ((not
     (memq
      (emacs-agent-changeset-status changeset)
      '(applied checkpointed reviewed)))
    (list :available nil :reason 'status))
   ((condition-case nil
        (cl-every
         (lambda (entry)
           (equal
            (emacs-agent-changeset--revision
             runtime (car entry))
            (cdr entry)))
         (emacs-agent-changeset-final-revisions changeset))
      (error nil))
    (list :available t))
   (t
    (list :available nil :reason 'revision_mismatch))))

(defun emacs-agent-changeset--revision-items (entries)
  "Convert revision ENTRIES to JSON-friendly plist items."
  (mapcar
   (lambda (entry)
     (list :path (car entry) :revision (cdr entry)))
   entries))

(defun emacs-agent-changeset-summary (runtime changeset)
  "Return a public summary of CHANGESET in RUNTIME."
  (let ((rollback
         (emacs-agent-changeset--rollback-status
          runtime changeset)))
    (list
     :changeset_id
     (emacs-agent-changeset-changeset-id changeset)
     :created_at
     (emacs-agent-changeset-created-at changeset)
     :status
     (emacs-agent-changeset-status changeset)
     :paths
     (copy-sequence
      (emacs-agent-changeset-touched-documents changeset))
     :operations
     (copy-tree (emacs-agent-changeset-operations changeset))
     :old_revisions
     (emacs-agent-changeset--revision-items
      (emacs-agent-changeset-base-revisions changeset))
     :new_revisions
     (emacs-agent-changeset--revision-items
      (emacs-agent-changeset-final-revisions changeset))
     :checkpointed
     (and
      (memq
       (emacs-agent-changeset-checkpoint-state changeset)
       '(t checkpointed))
      t)
     :rollback_available
     (plist-get rollback :available)
     :rollback_unavailable_reason
     (plist-get rollback :reason)
     :request_id
     (emacs-agent-changeset-request-id changeset)
     :agent_identity
     (emacs-agent-changeset-agent-identity changeset))))

(defun emacs-agent-changeset--new-cursor
    (runtime kind fingerprint data)
  "Store DATA for RUNTIME and KIND under FINGERPRINT."
  (let ((id (emacs-agent-runtime--random-id "cursor")))
    (puthash
     id
     (list
      :runtime-instance-id
      (emacs-agent-runtime-instance-id runtime)
      :kind kind
      :fingerprint fingerprint
      :data data
      :expires-at
      (+ (float-time) emacs-agent-changeset-cursor-ttl))
     emacs-agent-changeset-cursors)
    id))

(defun emacs-agent-changeset--consume-cursor
    (runtime cursor kind fingerprint)
  "Consume CURSOR for RUNTIME, KIND, and FINGERPRINT."
  (let ((state
         (and
          (stringp cursor)
          (gethash cursor emacs-agent-changeset-cursors))))
    (remhash cursor emacs-agent-changeset-cursors)
    (unless
        (and
         state
         (> (plist-get state :expires-at) (float-time))
         (equal
          (plist-get state :runtime-instance-id)
          (emacs-agent-runtime-instance-id runtime))
         (eq (plist-get state :kind) kind)
         (equal
          (plist-get state :fingerprint)
          fingerprint))
      (signal
       'emacs-agent-changeset-error
       (list "Invalid or expired change-set cursor")))
    (plist-get state :data)))

(cl-defun emacs-agent-changeset-query
    (runtime &key path statuses (limit 50) cursor)
  "Return recorded operations from RUNTIME filtered by PATH and STATUSES.
PATH, when non-nil, is an absolute direct-file target.  LIMIT bounds one
immutable page and CURSOR resumes the next page."
  (unless (and (integerp limit) (> limit 0) (<= limit 200))
    (signal
     'emacs-agent-changeset-error
     (list "Change-set limit must be between 1 and 200")))
  (let* ((canonical-path
          (and
           path
           (emacs-agent-changeset--canonical-path
            runtime path)))
         (fingerprint
          (secure-hash
           'sha256
           (prin1-to-string
            (list canonical-path statuses limit))))
         (state
          (and
           cursor
           (emacs-agent-changeset--consume-cursor
            runtime cursor 'list fingerprint)))
         (items
          (or
           (plist-get state :items)
           (seq-filter
            (lambda (changeset)
              (and
               (or
                (null canonical-path)
                (member
                 canonical-path
                 (emacs-agent-changeset-touched-documents
                  changeset)))
               (or
                (null statuses)
                (memq
                 (emacs-agent-changeset-status changeset)
                 statuses))))
            (emacs-agent-changeset-list runtime))))
         (offset (or (plist-get state :offset) 0))
         (end (min (length items) (+ offset limit)))
         (page (cl-subseq items offset end))
         (next
          (and
           (< end (length items))
           (emacs-agent-changeset--new-cursor
            runtime 'list fingerprint
            (list :items items :offset end)))))
    (list
     :changesets
     (mapcar
      (lambda (changeset)
        (emacs-agent-changeset-summary runtime changeset))
      page)
     :result_count (length page)
     :truncated (and next t)
     :cursor next)))

(cl-defun emacs-agent-changeset-detail
    (runtime changeset-id &key (max-chars (* 256 1024)) cursor)
  "Return CHANGESET-ID detail from RUNTIME with paginated diff."
  (unless (and (integerp max-chars) (> max-chars 0))
    (signal
     'emacs-agent-changeset-error
     (list "max_chars must be positive")))
  (let* ((fingerprint
          (secure-hash
           'sha256
           (prin1-to-string
            (list changeset-id max-chars))))
         (state
          (and
           cursor
           (emacs-agent-changeset--consume-cursor
            runtime cursor 'diff fingerprint)))
         (changeset
          (emacs-agent-changeset-get runtime changeset-id))
         (diff
          (or
           (plist-get state :diff)
           (emacs-agent-changeset-diff
            runtime changeset-id)))
         (offset (or (plist-get state :offset) 0))
         (end (min (length diff) (+ offset max-chars)))
         (next
          (and
           (< end (length diff))
           (emacs-agent-changeset--new-cursor
            runtime 'diff fingerprint
            (list :diff diff :offset end)))))
    (append
     (emacs-agent-changeset-summary runtime changeset)
     (list
      :diff (substring diff offset end)
      :diff_truncated (and next t)
      :diff_cursor next
      :diagnostics_before
      (emacs-agent-changeset-diagnostics-before changeset)
      :diagnostics_after
      (emacs-agent-changeset-diagnostics-after changeset)))))

(defun emacs-agent-changeset-diff (runtime &optional changeset-id)
  "Return a unified diff for CHANGESET-ID in RUNTIME.
When CHANGESET-ID is nil, concatenate all active change-set diffs."
  (let ((changesets
         (if changeset-id
             (list
              (emacs-agent-changeset-get
               runtime changeset-id))
           (seq-filter
            (lambda (changeset)
              (memq
               (emacs-agent-changeset-status changeset)
               '(applied checkpointed reviewed)))
            (reverse
             (emacs-agent-changeset-list runtime))))))
    (mapconcat
     (lambda (changeset)
       (or
        (emacs-agent-changeset-unified-diff changeset)
        (let ((diff
               (mapconcat
                (lambda (entry)
                  (let ((path (car entry))
                        (snapshot (cdr entry)))
                    (emacs-agent-changeset--diff-text
                     path
                     (emacs-agent-changeset--snapshot-content
                      snapshot)
                     (emacs-agent-changeset--current-content
                      runtime path))))
                (emacs-agent-changeset-before-snapshots
                 changeset)
                "")))
          (setf
           (emacs-agent-changeset-unified-diff changeset)
           diff)
          diff)))
     changesets "")))

(defun emacs-agent-changeset-mark-reviewed
    (runtime changeset-id)
  "Mark CHANGESET-ID in RUNTIME as reviewed."
  (let ((changeset
         (emacs-agent-changeset-get runtime changeset-id)))
    (when
        (eq
         (emacs-agent-changeset-status changeset)
         'rolled-back)
      (signal
       'emacs-agent-changeset-error
       (list "A rolled-back change set cannot be reviewed")))
    (setf
     (emacs-agent-changeset-status changeset)
     'reviewed)
    changeset))

(defun emacs-agent-changeset--validate-rollback
    (runtime changeset)
  "Validate every revision guard for CHANGESET in RUNTIME."
  (unless
      (memq
       (emacs-agent-changeset-status changeset)
       '(applied checkpointed reviewed))
    (signal
     'emacs-agent-rollback-conflict
     (list "Change set is not rollback-compatible")))
  (dolist
      (entry
       (emacs-agent-changeset-final-revisions changeset))
    (let ((current
           (emacs-agent-changeset--revision
            runtime (car entry))))
      (unless (equal current (cdr entry))
        (setf
         (emacs-agent-changeset-status changeset)
         'conflicted)
        (signal
         'emacs-agent-rollback-conflict
         (list
          (format
           "Document changed after change set: %s"
           (car entry))))))))

(defun emacs-agent-changeset--restore-one (runtime entry)
  "Restore one canonical before-image ENTRY in RUNTIME."
  (let* ((path (car entry))
         (snapshot (cdr entry))
         (target (emacs-agent-changeset--target runtime path))
         (canonical
          (emacs-agent-resolved-target-canonical-path target))
         (registry
          (emacs-agent-runtime-document-registry runtime))
         (document (gethash canonical registry))
         (buffer
          (and
           document
           (emacs-agent-document-buffer document))))
    (if (emacs-agent-changeset--snapshot-exists-p snapshot)
        (let* ((expected
                (emacs-agent-changeset--snapshot-content snapshot))
               (document
                (emacs-agent-document-open runtime target t))
               (buffer
                (emacs-agent-document-buffer document)))
          (with-current-buffer buffer
            (let ((inhibit-read-only t))
              (atomic-change-group
                (erase-buffer)
                (insert expected)))
            (when
                (eq
                 (emacs-agent-runtime-save-policy runtime)
                 'immediate)
              (emacs-agent-document-checkpoint document)
              (unless
                  (equal
                   expected
                   (emacs-agent-document--buffer-content buffer))
                (let ((inhibit-read-only t))
                  (atomic-change-group
                    (erase-buffer)
                    (insert expected)))
                (emacs-agent-signal
                 'checkpoint_failed
                 :path canonical
                 :reason 'rollback_snapshot_changed_by_save_hook
                 :checkpointed nil
                 :partial_completion t
                 :reconciliation_required t
                 :filesystem_rollback_guaranteed nil)))))
      (when (buffer-live-p buffer)
        (with-current-buffer buffer
          (set-buffer-modified-p nil))
        (kill-buffer buffer))
      (remhash canonical registry)
      (when (file-exists-p canonical)
        (delete-file canonical)))))

(defun emacs-agent-changeset-rollback (runtime changeset-id)
  "Rollback CHANGESET-ID in RUNTIME when final revisions still match.
The complete guard and target-resolution passes occur before any mutation.
Under the `immediate' save policy, restored buffers are checkpointed."
  (let ((changeset
         (emacs-agent-changeset-get runtime changeset-id)))
    (emacs-agent-changeset--validate-rollback
     runtime changeset)
    ;; Resolve every target before the first mutation, then resolve it again
    ;; immediately before use to catch replaced symlink parents.
    (dolist
        (entry
         (emacs-agent-changeset-before-snapshots changeset))
      (emacs-agent-changeset--target runtime (car entry)))
    (emacs-agent-runtime-enqueue-mutation
     runtime
     (lambda ()
       (let (restoring-entry restored-paths)
         (condition-case error-data
             (progn
               (dolist
                   (entry
                    (emacs-agent-changeset-before-snapshots
                     changeset))
                 (setq restoring-entry entry)
                 (emacs-agent-changeset--restore-one
                  runtime entry)
                 (push (car entry) restored-paths))
               (setf
                (emacs-agent-changeset-status changeset)
                'rolled-back)
               (emacs-agent-runtime-record-activity
                runtime
                (list
                 :tool "changeset_rollback"
                 :status "completed"
                 :changeset_id changeset-id
                 :paths
                 (emacs-agent-changeset-touched-documents
                  changeset)))
               changeset)
           (error
            (when restoring-entry
              (let* ((path (car restoring-entry))
                     (target
                      (ignore-errors
                        (emacs-agent-changeset--target
                         runtime path)))
                     (canonical
                      (and
                       target
                       (emacs-agent-resolved-target-canonical-path
                        target)))
                     (document
                      (and
                       canonical
                       (gethash
                        canonical
                        (emacs-agent-runtime-document-registry
                         runtime))))
                     (buffer
                      (and
                       document
                       (emacs-agent-document-buffer document))))
                (when document
                  (setf
                   (emacs-agent-document-degraded document)
                   t))
                (when (buffer-live-p buffer)
                  (with-current-buffer buffer
                    (set-buffer-modified-p t)))))
            (setf
             (emacs-agent-changeset-status changeset)
             'conflicted
             (emacs-agent-runtime-health-state runtime)
             'degraded)
            (let ((completed (nreverse restored-paths))
                  (failed
                   (and restoring-entry
                        (car restoring-entry))))
              (emacs-agent-runtime-record-activity
               runtime
               (list
                :tool "changeset_rollback"
                :status "partial"
                :changeset_id changeset-id
                :restored_paths completed
                :failed_path failed))
              (signal
               'emacs-agent-rollback-conflict
               (list
                (format
                 "Rollback partially completed before %s failed: %s"
                 failed
                 (error-message-string error-data))
                :partial_completion t
                :restored_paths completed
                :failed_path failed
                :reconciliation_required t))))))))))

(provide 'emacs-agent-changeset)
;;; emacs-agent-changeset.el ends here
