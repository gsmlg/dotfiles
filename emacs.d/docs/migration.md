...
startup follows the local Emacs server lifecycle by default and can be
disabled with `gsmlg-agent-autostart`. Version 0.3 removes the legacy startup
directory setting and starts a project-optional editor runtime without
consulting `default-directory`.
