...
(ert-deftest gsmlg-envrc-reload-and-refresh-eglot-reconnects-managed ()
  "Envrc reload clears the negative cache and reconnects a managed buffer."
  (let ((reloaded nil)
        (reconnected nil)
        (cleared nil)
        (gsmlg-eglot-supported-modes '(python-mode))
        (major-mode 'python-mode)
        (default-directory "/project/"))
    (puthash (gsmlg-eglot--cache-key) t gsmlg-eglot-unavailable-cache)
    (cl-letf (((symbol-function #'envrc-reload)
              ...
              ((symbol-function #'eglot-reconnect)
               (lambda (server &optional _interactive)
                 (should (eq server 'server))
                 (setq reconnected t))))
      (gsmlg-envrc-reload-and-refresh-eglot)
      (setq cleared (not (gethash (gsmlg-eglot--cache-key)
                                  gsmlg-eglot-unavailable-cache))))
    (should reloaded)
    (should reconnected)
    (should cleared)
...
(ert-deftest gsmlg-envrc-reload-and-refresh-eglot-ensures-unmanaged ()
  "Envrc reload starts Eglot when the buffer is not yet managed."
  (let ((ensured nil)
        (gsmlg-eglot-supported-modes '(python-mode))
        (major-mode 'python-mode))
    (cl-letf (((symbol-function #'envrc-reload) #'ignore)
              ((symbol-function #'eglot-managed-p) (lambda () nil))
              ((symbol-function #'gsmlg-eglot-ensure-maybe)
