...
(ert-deftest gsmlg-agent-reconcile-is-idempotent ()
  "Repeated reconcile calls share one state machine and do not nest."
  (let ((noninteractive nil)
        (gsmlg-agent-autostart t)
        (gsmlg-agent-package-available-p t)
        (server-mode t)
        (server-process (list 'fake-server))
        (gsmlg-agent-state 'available)
        (starts 0))
    (cl-letf (((symbol-function #'process-live-p)
               (lambda (_process) t))
              ((symbol-function #'emacs-agent-editor-running-p)
               (lambda () (eq gsmlg-agent-state 'running)))
              ((symbol-function #'emacs-agent-editor-start)
               (lambda (_port)
                 (setq starts (1+ starts)
                       gsmlg-agent-state 'running)
                 'server)))
      (should (eq (gsmlg-agent-reconcile) 'server))
      (should (eq (gsmlg-agent-reconcile) 'running))
      (should (= starts 1)))))
